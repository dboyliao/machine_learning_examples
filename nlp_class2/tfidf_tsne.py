...
from rnn_class.brown import (get_sentences_with_word2idx,
...
def main():
    ...
    sentences, word2idx = get_wikipedia_data(
        n_files=3, n_vocab=2000, by_paragraph=True, prefix="./largefiles/"
    )
    ...
    for word_list in analogies_to_try:
        for w in word_list:
            if w not in word2idx:
                print(
                    "%s not found in vocab, remove it from \
    ...
    for i in range(V):
        try:
            plt.annotate(
                s=idx2word[i].encode("utf8").decode("utf8"), xy=(Z[i, 0], Z[i, 1])
            )
        except:
